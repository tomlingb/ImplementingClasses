...
class Point(object):
    ...
    def move_to(self, x, y):
        ...
        distance = math.sqrt((self.x - prevx) ** 2 +
                             (self.y - prevy) ** 2)
        self.distance_trav = self.distance_trav + distance
    ...
    def get_distance_from(self, point):
        distance = math.sqrt((point.x - self.x) ** 2 +
                             (point.y - self.y) ** 2)
        self.distance_trav = self.distance_trav + distance
        return distance

    def get_distance_from_start(self):
        distance_from_start = math.sqrt((self.startx - self.x) ** 2 +
                                        (self.starty - self.y) ** 2)
        return distance_from_start

    def get_distance_traveled(self):
        return self.distance_trav

    def closer_to(self, p2, p3):
        distance1 = math.sqrt((p2.x - self.x) ** 2 +
                              (p2.y - self.y) ** 2)
        distance2 = math.sqrt((p3.x - self.x) ** 2 +
                              (p3.y - self.y) ** 2)
